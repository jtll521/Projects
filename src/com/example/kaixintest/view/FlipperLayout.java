...
public class FlipperLayout extends ViewGroup {
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		Log.i("jiangtao4", "onLayout");
		for (int i = 0; i < getChildCount(); i++) {
			View child = getChildAt(i);
			int height = child.getMeasuredHeight();
			int width = child.getMeasuredWidth();
			child.layout(0, 0, width, height);
			Log.i("jiangtao4", "onLayout:"+"height"+height+"width"+width);
		}
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		Log.i("jiangtao4", "onMeasure");
		int width = MeasureSpec.getSize(widthMeasureSpec);
		int height = MeasureSpec.getSize(heightMeasureSpec);
		setMeasuredDimension(width, height);
		for (int i = 0; i < getChildCount(); i++) {
			getChildAt(i).measure(widthMeasureSpec, heightMeasureSpec);
			Log.i("jiangtao4", "onMeasure:"+"widthMeasureSpec"+width+
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_DOWN:
			Log.i("jiangtao4", "dispatchTouchEvent-->ACTION_DOWN");
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		obtainVelocityTracker(ev);
		Log.i("jiangtao4", "onInterceptTouchEvent");
		switch (ev.getAction()) {
		case MotionEvent.ACTION_DOWN:
			Log.i("jiangtao4", "ACTION_DOWN");
			mTouchState = mScroller.isFinished() ? TOUCH_STATE_RESTART
					: TOUCH_STATE_SCROLLING;
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		Log.i("jiangtao4", "onTouchEvent");
		switch (event.getAction()) {
		case MotionEvent.ACTION_DOWN:
			Log.i("jiangtao4", "MotionEvent.ACTION_DOWN");
			mTouchState = mScroller.isFinished() ? TOUCH_STATE_RESTART
					: TOUCH_STATE_SCROLLING;
			if (mTouchState == TOUCH_STATE_SCROLLING) {
				return false;
		...
		case MotionEvent.ACTION_MOVE:
			Log.i("jiangtao4", "MotionEvent.ACTION_MOVE");
			mVelocityTracker.computeCurrentVelocity(1000,
					ViewConfiguration.getMaximumFlingVelocity());
		...
		case MotionEvent.ACTION_UP:
			Log.i("jiangtao4", "MotionEvent.ACTION_UP");
			if (mScrollState == SCROLL_STATE_ALLOW) {
	...
	private void obtainVelocityTracker(MotionEvent event) {
		Log.i("jiangtao4", "obtainVelocityTracker");
	...
	private void releaseVelocityTracker() {
		Log.i("jiangtao4", "releaseVelocityTracker");
		if (mVelocityTracker != null) {
